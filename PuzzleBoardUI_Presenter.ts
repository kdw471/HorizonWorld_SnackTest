...
export class PuzzleBoardPresenter {
	...
	private _cellBaselines: (PuzzleBoardCellView | undefined)[] = [];
	private readonly _touchedCells: number[] = [];
	private _sideBaselines: (PuzzleBoardCellView | undefined)[] = [];
	private readonly _touchedSideCells: number[] = [];
	private _itemBaselines: (PuzzleBoardItemView | undefined)[] = [];
	private readonly _touchedItems: number[] = [];
	...
	public setCell(index: number, patch: PuzzleBoardCellPatch): boolean {
		...
		if (this._batchDepth > 0) {
			if (this._cellBaselines[index] === undefined) {
				this._cellBaselines[index] = current;
				this._touchedCells.push(index);
			}
			return true;
		}
	...
	public setSideCell(index: number, patch: PuzzleBoardCellPatch): boolean {
		...
		if (this._batchDepth > 0) {
			if (this._sideBaselines[index] === undefined) {
				this._sideBaselines[index] = current;
				this._touchedSideCells.push(index);
			}
			return true;
		}
	...
	public setItem(index: number, patch: PuzzleBoardItemPatch): boolean {
		...
		if (this._batchDepth > 0) {
			if (this._itemBaselines[index] === undefined) {
				this._itemBaselines[index] = current;
				this._touchedItems.push(index);
			}
			return true;
		}
	...
	public endBatch(): void {
		if (this._batchDepth === 0) {
			console.warn('[PuzzleBoardPresenter] endBatch() was called without a matching beginBatch().');
			return;
		}
		this._batchDepth--;
		if (this._batchDepth > 0) {
	...
	private flushBatch(): void {
		const cells = this._view.grid.cells;
		for (const index of this._touchedCells) {
			const baseline = this._cellBaselines[index];
			this._cellBaselines[index] = undefined;
			const current = cells[index];
			if (baseline === undefined || current === undefined || isSameCellView(baseline, current)) {
				continue;
			}
			this.CELL_CHANGED.publish({ index: index, cell: current });
		}
		this._touchedCells.length = 0;

		const side = this._view.side;
		for (const index of this._touchedSideCells) {
			const baseline = this._sideBaselines[index];
			this._sideBaselines[index] = undefined;
			const current = side?.cells[index];
			if (baseline === undefined || current === undefined || isSameCellView(baseline, current)) {
				continue;
			}
			this.SIDE_CELL_CHANGED.publish({ index: index, cell: current });
		}
		this._touchedSideCells.length = 0;

		const items = this._view.items;
		for (const index of this._touchedItems) {
			const baseline = this._itemBaselines[index];
			this._itemBaselines[index] = undefined;
			const current = items[index];
			if (baseline === undefined || current === undefined || isSameItemView(baseline, current)) {
				continue;
			}
			this.ITEM_CHANGED.publish({ index: index, item: current });
		}
		this._touchedItems.length = 0;
	}

	/** 규격 크기에 맞춰 baseline 버퍼를 새로 잡고, 기록 중이던 자리를 모두 버린다 */
	private resetBatchBuffers(): void {
		this._cellBaselines = new Array<PuzzleBoardCellView | undefined>(this._view.grid.cells.length);
		this._touchedCells.length = 0;
		this._sideBaselines = new Array<PuzzleBoardCellView | undefined>(this._view.side?.cells.length ?? 0);
		this._touchedSideCells.length = 0;
		this._itemBaselines = new Array<PuzzleBoardItemView | undefined>(this._view.items.length);
		this._touchedItems.length = 0;
	}
	...
	private warnOnLostRelease(kind: string): void {
		this._lostReleaseCount++;
		console.warn(`[PuzzleBoardPresenter] A ${kind} press arrived while the previous press was still open, `
			+ `so the previous release was lost (count ${this._lostReleaseCount}). `
			+ 'The stuck drag is finalized on this press instead.');
	}
