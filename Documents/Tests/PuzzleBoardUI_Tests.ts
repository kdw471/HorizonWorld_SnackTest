...
function testDefinitions(recorder: TestRecorder): void {
	...
	// 여러 색 띠 (레이저의 겹친 광선)
	const red = boardColor(1, 0, 0);
	const blue = boardColor(0, 0, 1);
	recorder.check('칸 - 띠의 초기값은 없음', cell.stripes.length === 0);
	recorder.check('칸 - 빈 띠를 다시 주면 패치 결과가 undefined',
		applyCellPatch(cell, { stripes: NO_STRIPES }) === undefined && applyCellPatch(cell, { stripes: [] }) === undefined);
	const striped = applyCellPatch(cell, { stripes: [red, blue], stripeOrientation: EBoardStripeOrientation.VERTICAL });
	recorder.check('칸 - 띠를 주면 새 스냅샷을 돌려준다',
		striped !== undefined && striped.stripes.length === 2 && striped.stripeOrientation === EBoardStripeOrientation.VERTICAL);
	recorder.check('칸 - 같은 색 띠(다른 배열)를 다시 주면 패치 결과가 undefined',
		striped !== undefined && applyCellPatch(striped, { stripes: [boardColor(1, 0, 0), boardColor(0, 0, 1)] }) === undefined);
	recorder.check('칸 - 띠 순서가 다르면 다른 스냅샷으로 본다',
		striped !== undefined && isSameCellView(striped, { ...striped, stripes: [blue, red] }) === false);
	recorder.check('칸 - 띠 개수가 다르면 다른 스냅샷으로 본다',
		striped !== undefined && isSameCellView(striped, { ...striped, stripes: [red] }) === false);
	recorder.check('칸 - 띠 방향만 달라도 다른 스냅샷으로 본다',
		striped !== undefined && isSameCellView(striped, { ...striped, stripeOrientation: EBoardStripeOrientation.HORIZONTAL }) === false);
	recorder.check('칸 - 띠를 비우면 다시 fill 하나로 돌아간다',
		striped !== undefined && applyCellPatch(striped, { stripes: NO_STRIPES })?.stripes.length === 0);
...
/** 리페인트 배치 커밋 - 배치 안의 덧칠은 새지 않고, 닫힐 때 실제로 달라진 자리만 1회 알린다 */
function testPresenterBatch(recorder: TestRecorder): void {
	const presenter = new PuzzleBoardPresenter({ title: '', rowCount: 3, colCount: 3, itemCount: 2 });
	const cellLog: number[] = [];
	const last: { cell: PuzzleBoardCellView | undefined } = { cell: undefined };
	presenter.CELL_CHANGED.subscribe((change) => {
		cellLog.push(change.index);
		last.cell = change.cell;
	});
	let itemChangeCount = 0;
	presenter.ITEM_CHANGED.subscribe(() => { itemChangeCount++; });

	presenter.setCell(3, { isVisible: true, label: 'A' });
	recorder.check('배치 - 배치 밖의 갱신은 즉시 알린다', cellLog.length === 1 && cellLog[0] === 3);

	// A -> B -> A: 바탕 리셋 뒤 원래대로 덧칠하는 페인터의 전형이다. 밖에서는 아무것도 바뀌지 않았다
	let insideCount = -1;
	presenter.runBatch(() => {
		presenter.setCell(3, { label: '' });
		presenter.setCell(3, { label: 'A' });
		insideCount = cellLog.length;
	});
	recorder.check('배치 - 배치 중에는 이벤트가 나가지 않는다', insideCount === 1);
	recorder.check('배치 - A→B→A 는 이벤트를 내지 않는다', cellLog.length === 1);
	recorder.check('배치 - 스냅샷은 최종값이다', presenter.getCell(3)?.label === 'A');
	recorder.check('배치 - 배치가 닫히면 더 이상 배치 중이 아니다', presenter.isBatching === false);

	// A -> B -> C: 두 번 패치해도 최종값으로 한 번만 알린다
	presenter.runBatch(() => {
		presenter.setCell(4, { isVisible: true });
		presenter.setCell(4, { label: 'C' });
	});
	const finalCell = last.cell;
	recorder.check('배치 - 여러 번 패치한 칸은 최종값으로 1회',
		cellLog.length === 2 && cellLog[1] === 4
		&& finalCell !== undefined && finalCell.isVisible === true && finalCell.label === 'C');

	// 여러 칸이 바뀌면 각각 한 번씩, 건드린 순서대로
	presenter.runBatch(() => {
		presenter.setCell(8, { isVisible: true });
		presenter.setCell(0, { isVisible: true });
		presenter.setCell(8, { label: 'H' });
	});
	recorder.check('배치 - 바뀐 칸마다 1회, 건드린 순서대로',
		cellLog.length === 4 && cellLog[2] === 8 && cellLog[3] === 0);

	// 중첩 - 안쪽이 닫혀도 바깥이 열려 있으면 미룬다
	let afterInner = -1;
	presenter.runBatch(() => {
		presenter.runBatch(() => {
			presenter.setCell(1, { isVisible: true });
		});
		afterInner = cellLog.length;
	});
	recorder.check('배치 - 중첩 배치는 가장 바깥이 닫힐 때 알린다',
		afterInner === 4 && cellLog.length === 5 && cellLog[4] === 1);

	// 예외 - 페인터가 던져도 깊이가 복구되고, 그때까지의 변경은 알린다
	let isThrown = false;
	try {
		presenter.runBatch(() => {
			presenter.setCell(2, { isVisible: true });
	...
	recorder.check('배치 - 예외가 나도 배치가 닫히고 변경은 알린다',
		isThrown && presenter.isBatching === false && cellLog.length === 6 && cellLog[5] === 2);
	presenter.setCell(2, { label: 'X' });
	recorder.check('배치 - 예외 뒤의 갱신은 즉시 알린다', cellLog.length === 7);

	// 짝 없는 endBatch 는 무시된다 (경고만) - 이후 갱신은 즉시 나간다
	presenter.endBatch();
	presenter.setCell(2, { label: 'Y' });
	recorder.check('배치 - 짝 없는 endBatch 는 무시된다', presenter.isBatching === false && cellLog.length === 8);

	// 배치 중 규격 교체 - 옛 뷰의 baseline 은 버리고, 새 뷰의 칸만 알린다
	let layoutCount = 0;
	presenter.LAYOUT_CHANGED.subscribe(() => { layoutCount++; });
	presenter.runBatch(() => {
		presenter.setCell(5, { isVisible: true });
		presenter.resetLayout({ title: '', rowCount: 2, colCount: 2 });
		presenter.setCell(0, { isVisible: true });
		presenter.setCell(0, { isVisible: false });
		presenter.setCell(1, { isVisible: true });
	});
	recorder.check('배치 - 배치 중 규격 교체는 옛 칸을 버리고 새 칸만 알린다',
		layoutCount === 1 && cellLog.length === 9 && cellLog[8] === 1
		&& presenter.getCell(0)?.isVisible === false && presenter.getCell(1)?.isVisible === true);

	// 트레이 슬롯도 같은 규칙
	const tray = new PuzzleBoardPresenter({ title: '', rowCount: 2, colCount: 2, itemCount: 2 });
	tray.ITEM_CHANGED.subscribe(() => { itemChangeCount++; });
	tray.setItem(0, { isVisible: true, label: 'P' });
	tray.runBatch(() => {
		tray.setItem(0, { label: '' });
		tray.setItem(0, { label: 'P' });
		tray.setItem(1, { isVisible: true });
	});
	recorder.check('배치 - 트레이 슬롯도 달라진 것만 1회', itemChangeCount === 2 && tray.getItem(1)?.isVisible === true);

	// 보조 격자도 같은 규칙
	const side = new PuzzleBoardPresenter(SWITCH_LAYOUT);
	let sideCount = 0;
	side.SIDE_CELL_CHANGED.subscribe(() => { sideCount++; });
	side.runBatch(() => {
		side.setSideCell(0, { isVisible: true });
		side.setSideCell(0, { isVisible: false });
		side.setSideCell(1, { isVisible: true });
	});
	recorder.check('배치 - 보조 격자도 달라진 것만 1회', sideCount === 1);
}
