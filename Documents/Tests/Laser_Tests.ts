...
import { LaserBeamTracer, summarizeBeamCells } from 'Laser_BeamTracer';
...
//#region 겹친 광선의 칸 단위 요약 (연출용)

function testBeamCellSummary(recorder: TestRecorder): void {
	const tracer = new LaserBeamTracer();

	// 빨강(가로)과 초록(세로)이 (3,3) 에서 교차한다 - 교차 칸만 두 색, 나머지는 한 색
	{
		const board = makeBoard([
			{ id: 'E1', type: EGimmickType.EMITTER, row: 3, col: 0, colors: [ELaserColor.RED] },
			{ id: 'R1', type: EGimmickType.RECEIVER, row: 3, col: 6, colors: [ELaserColor.RED] },
		...
		const cells = summarizeBeamCells(tracer.trace(board).segments);
		const crossing = cells.find((cell) => cell.row === 3 && cell.col === 3);
		const redOnly = cells.find((cell) => cell.row === 3 && cell.col === 1);
		const greenOnly = cells.find((cell) => cell.row === 1 && cell.col === 3);

		recorder.check('요약: 교차 칸은 두 색을 모두 담는다',
			crossing !== undefined && crossing.colors.length === 2
			&& crossing.colors.indexOf(ELaserColor.RED) >= 0 && crossing.colors.indexOf(ELaserColor.GREEN) >= 0,
			crossing?.colors.join());
		recorder.check('요약: 교차 칸은 두 진행 방향을 모두 담는다',
			crossing !== undefined && crossing.directions.length === 2
			&& crossing.directions.indexOf(ELaserDirection.RIGHT) >= 0 && crossing.directions.indexOf(ELaserDirection.DOWN) >= 0,
			crossing?.directions.join());
		recorder.check('요약: 한 색만 지나는 칸은 색이 하나',
			redOnly?.colors.length === 1 && redOnly.colors[0] === ELaserColor.RED
			&& greenOnly?.colors.length === 1 && greenOnly.colors[0] === ELaserColor.GREEN);
		recorder.check('요약: 같은 칸은 한 번만 나온다',
			new Set(cells.map((cell) => `${cell.row},${cell.col}`)).size === cells.length);
	}

	// 같은 경로를 두 색이 나란히 지나면 모든 칸이 두 색 - 발사체 두 개가 같은 줄에 있을 수는
	// 없으므로 세그먼트를 직접 만든다. 추적 순서와 무관하게 색은 표준 순서(RED, GREEN, BLUE)다.
	{
		const cells = summarizeBeamCells([
			{ color: ELaserColor.BLUE, from: { row: 2, col: 1 }, to: { row: 2, col: 2 }, direction: ELaserDirection.RIGHT },
			{ color: ELaserColor.RED, from: { row: 2, col: 1 }, to: { row: 2, col: 2 }, direction: ELaserDirection.RIGHT },
			{ color: ELaserColor.GREEN, from: { row: 2, col: 1 }, to: { row: 2, col: 2 }, direction: ELaserDirection.RIGHT },
			// 같은 색이 다른 방향으로 다시 지나도 색은 중복되지 않는다
			{ color: ELaserColor.RED, from: { row: 2, col: 3 }, to: { row: 2, col: 2 }, direction: ELaserDirection.LEFT },
		]);
		const shared = cells.find((cell) => cell.row === 2 && cell.col === 2);
		recorder.check('요약: 색은 추적 순서가 아니라 표준 순서로 정렬된다',
			shared !== undefined && shared.colors.join() === [ELaserColor.RED, ELaserColor.GREEN, ELaserColor.BLUE].join(),
			shared?.colors.join());
		recorder.check('요약: 같은 색이 여러 번 지나도 한 번만 담는다',
			shared !== undefined && shared.colors.length === 3 && shared.directions.length === 2,
			`${shared?.colors.length} colors / ${shared?.directions.length} directions`);
		recorder.check('요약: 세그먼트의 양 끝 칸이 모두 담긴다',
			cells.some((cell) => cell.row === 2 && cell.col === 1) && cells.some((cell) => cell.row === 2 && cell.col === 3));
	}

	recorder.check('요약: 세그먼트가 없으면 빈 목록', summarizeBeamCells([]).length === 0);
}
