...
export class ColorSortCoreAPI extends Component<typeof ColorSortCoreAPI> {
	...
	private constructSystems(): void {
		...
		this._dragStream = new PuzzleScreenDragStream(
			this, CASE_CAPACITY, this._caseCount, {
		...
		console.log('[ColorSortCoreAPI] Drag stream created '
	...
	private resizeGridToActiveCases(): void {
		...
		// 화면 -> 격자 변환도 패널이 그리는 열 수를 따라가야 손가락과 같은 케이스가 잡힌다
		this._dragStream?.setGridSize(CASE_CAPACITY, nextCount);
	...
	private onDragBegin(caseIndex: number): void {
		...
		// 잡는 순간 바뀌는 열은 출발 케이스 하나뿐이다 (건전지가 떠오르고 실루엣이 남는다)
		this.applyCaseVisual(result.fromCaseIndex);
	}
	...
	private onDragMove(caseIndex: number | undefined): void {
		...
		this.trackDragMoveTo(caseIndex);
	}

	/** 이동의 본체 - 칸 단위 경로와 스트림 경로가 합류한다 */
	private trackDragMoveTo(caseIndex: number | undefined): void {
	 ...
	 * 열이 곧 케이스이므로 열만 반올림한다 (`toCaseFromPoint`).
	 */
	private onStreamDragMove(point: PuzzleGridPoint): void {
		if (this._dragFromCase === undefined) {
			return;
		}
		this.trackDragMoveTo(this.toCaseFromPoint(point));
	}
	...
	private onStreamDragEnd(point: PuzzleGridPoint): void {
		...
		// 좌표를 만들 수 없었던 뗌(NaN)은 마지막 이동이 반영한 케이스에 그대로 확정한다
		const caseIndex = isNaN(point.row) || isNaN(point.col)
			? this._hoverCase
			: this.toCaseFromPoint(point);
		this.finalizeDrag(caseIndex);
	}

	/**
	 * 연속 격자 좌표 -> 케이스 index. 영역 밖이면 undefined.
	 *
	 * 케이스는 열이므로 **열만 반올림**한다. 세로는 격자 위아래로 한 칸까지 너그럽게 본다 -
	 * 건전지를 집어 올려 케이스 위쪽 허공으로 끌고 가는 것이 자연스러운 손짓이고, 그 자리에서
	 * 놓았다고 "영역 밖 드랍"(2초 잠금)을 물리면 조작이 억울하게 느껴진다.
	 * 그보다 멀리(다른 레이아웃 영역) 나간 것만 영역 밖이다.
	 */
	private toCaseFromPoint(point: PuzzleGridPoint): number | undefined {
		const col = Math.round(point.col);
		const row = Math.round(point.row);
		if (col < 0 || col >= this._caseCount || row < -1 || row > CASE_CAPACITY) {
			return undefined;
		}
		return col;
	}
	...
	private onDragEnd(caseIndex: number | undefined): void {
		...
		this.finalizeDrag(this._dragStream?.isDriving === true ? this._hoverCase : caseIndex);
	}

	/**
	 * 릴리즈 확정의 본체 - 칸 단위 경로(`onDragEnd`)와 스트림 경로(`onStreamDragEnd`)가 합류한다.
	 * 뗀 케이스를 마지막으로 한 번 더 반영한 뒤 확정한다.
	 */
	private finalizeDrag(caseIndex: number | undefined): void {
		this.trackDragTo(caseIndex);
