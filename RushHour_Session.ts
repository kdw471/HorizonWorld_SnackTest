...
export class RushHourSession {
	...
	/** 터치 기준점 보정 - 화면 좌표 스트림이 실제 터치 지점을 알려 줄 때 (`RushHourDragController.rebaseTouchOrigin`) */
	public rebaseDragOrigin(gridRow: number, gridCol: number): boolean {
		if (this._dragController === undefined) {
			return false;
		}
		return this._dragController.rebaseTouchOrigin(gridRow, gridCol);
	}
