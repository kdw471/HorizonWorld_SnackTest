...
/**
 * 칸을 여러 색 띠로 나눠 칠하는 층 - 색이 다른 광선이 **같은 칸을 같이 지날 때** 쓴다.
 *
 * `stripes` 가 비어 있지 않은 칸에만 올라간다. 띠는 색의 개수만큼 **같은 비율**로
 * 칸을 나누고(`flex: 1`), 쓰지 않는 띠는 `display: none` 으로 자리를 비워 나머지가
 * 칸을 꽉 채운다. 그림(`createTextureLayer`)과 `fill` 위에 덮이므로 띠가 있는 동안은
 * 그것들이 가려진다 - "여기를 여러 색이 지난다" 가 그림보다 중요하기 때문이다.
 *
 * ## 방향은 회전으로 바꾼다
 *
 * `flexDirection` 은 Binding 을 받지 않는다. 그래서 띠를 언제나 **위에서 아래로**
 * 쌓아 두고(`column`), 세로 띠가 필요하면 층 전체를 270도 돌린다 - 칸이 정사각형이라
 * 돌려도 정확히 들어맞고, 위에 있던 첫 띠가 **왼쪽**으로 온다(반시계 90도). 띠마다
 * 위치·크기를 Binding 으로 주면 파생이 띠당 넷씩 늘어나므로, 층 하나의 회전 하나로
 * 대신한다 - 파생 Binding 의 수가 곧 패널의 갱신 비용이다 (`PuzzleBoardUI_Grid` 머리말).
 *
 * 노드는 `PUZZLE_BOARD_MAX_STRIPES` 개로 고정이다. Custom UI 트리는 한 번만
 * 만들어지므로 런타임에 띠를 늘릴 수 없다.
 */
export function createStripeLayer(
	metrics: BoardMetrics,
	binding: Binding<PuzzleBoardCellView>,
	cornerRadius: number,
): UINode {
	const hasStripes = binding.derive((cell) => cell.stripes.length > 0);

	const bands: UINode[] = [];
	for (let index = 0; index < PUZZLE_BOARD_MAX_STRIPES; index++) {
		bands.push(View({
			children: [],
			style: {
				flex: 1,
				width: '100%',
				display: binding.derive((cell) => (index < cell.stripes.length ? 'flex' : 'none')),
				backgroundColor: binding.derive((cell) => toColor(cell.stripes[index] ?? BOARD_COLOR_EMPTY)),
			},
		}));
	}

	return UINode.if(hasStripes, View({
		children: bands,
		style: {
			position: 'absolute',
			width: '100%',
			height: '100%',
			borderRadius: metrics.px(cornerRadius),
			// 띠는 네모라 둥근 모서리 밖으로 삐져나온다 - 얼굴 모양대로 잘라 낸다
			overflow: 'hidden',
			flexDirection: 'column',
			transform: [{
				rotate: binding.derive((cell) =>
					(cell.stripeOrientation === EBoardStripeOrientation.VERTICAL ? '270deg' : '0deg')),
			}],
		},
	}));
}
