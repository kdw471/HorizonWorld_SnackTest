...
/**
 * 색의 표준 순서. 여러 색을 한 줄로 늘어놓을 때(겹친 광선의 띠 등) 이 순서를 따른다 -
 * 어느 광선이 먼저 추적됐든 화면에는 언제나 같은 순서로 나와야 하기 때문이다.
 */
export const LASER_COLOR_ORDER: readonly ELaserColor[] = [ELaserColor.RED, ELaserColor.GREEN, ELaserColor.BLUE];
...
/**
 * 광선이 지나는 칸 하나의 요약 - 연출용 (`LaserBeamTracer.summarizeBeamCells`).
 *
 * 세그먼트는 색·갈래마다 따로 나오므로 같은 칸이 여러 번 등장한다. 연출은 칸 단위로
 * "여기를 어떤 색들이 지나는가" 만 알면 되므로 여기서 한 번 모아 준다.
 */
export type LaserBeamCellSummary = {
	/** 전체 그리드 좌표 */
	row: number,
	col: number,
	/** 이 칸을 지나는 색 - 중복 없이 `LASER_COLOR_ORDER` 순서다 */
	colors: ELaserColor[],
	/** 이 칸을 지나는 광선의 진행 방향 - 중복 없이, 처음 만난 순서다 */
	directions: ELaserDirection[],
}
