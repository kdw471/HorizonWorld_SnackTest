...
export class RushHourCoreAPI extends Component<typeof RushHourCoreAPI> {
	...
	private constructSystems(): void {
		...
		this._dragStream = new PuzzleScreenDragStream(
			this, RUSH_HOUR_FULL_GRID_SIZE, RUSH_HOUR_FULL_GRID_SIZE, {
				onStreamStart: (point) => this.onStreamDragStart(point),
	...
	private onDragBegin(cell: number): void {
		...
		// 잡는 순간 바뀌는 칸은 오브젝트의 발자국(GRABBED)과 이 축의 길(PATH)뿐이다.
		// 예전에는 여기서 81칸 전체를 다시 칠했다 - "터치 즉시 붙는다" 는 첫 피드백이
		// 가장 무거운 리페인트 뒤에 나왔던 셈이다. 릴리즈 더티 수집과 같은 범위다
		// (미리보기 자리 = 원래 자리이므로 정확히 발자국 + 길이 된다).
		this.applyDragFocusedVisuals();
	}

	/**
	 * 드래그의 발자국·길만 다시 기록한다 - 잡는 순간과 1x1 의 축 확정 순간에 쓴다.
	 * 둘 다 "오브젝트 자리 + 길" 밖은 바뀌지 않으므로 전체를 훑을 이유가 없다.
	 * 축 확정은 직전 미리보기 자리(비워진다)도 함께 넘긴다.
	 */
	private applyDragFocusedVisuals(previousRow?: number, previousCol?: number): void {
		const pieceId = this._previewPieceId;
		const piece = pieceId === undefined ? undefined : this.session.board?.getPiece(pieceId);
		if (piece === undefined || this.collectReleaseDirtyCells() === false) {
		...
		// 결합된 USB 는 발자국(2칸) 밖의 슬롯 칸까지 3칸을 차지한다 (§9). 잡으면 미리보기가
		// 발자국으로만 그려지므로 슬롯 칸도 다시 칠해야 예전 전체 리페인트와 같은 그림이 된다.
		for (const cell of this.session.board?.getGoalOccupiedCellsInFullGrid(piece.id) ?? []) {
			this._dirtyCells.add(cell.row * RUSH_HOUR_FULL_GRID_SIZE + cell.col);
		}
	...
	private trackDragToLocal(localRow: number, localCol: number): boolean {
		...
		if (didAxisChange) {
			// 축이 이 순간 확정됐다 - 이동 가능 범위(길)가 처음 나타난다. 바뀌는 칸은
			// 원래 자리·이전/새 미리보기 자리·이 축의 길뿐이다 (1x1 이라 발자국은 한 칸씩).
			this.applyDragFocusedVisuals(previousRow, previousCol);
		}
		else {
