...
//#region Stripes (한 칸을 여러 색으로 나눠 칠하기)

/**
 * 칸 하나를 **같은 비율의 띠 여러 개**로 나눠 칠할 때의 띠 방향.
 *
 * ## 왜 필요한가
 *
 * 레이저 퍼즐에서 색이 다른 광선은 층이 달라 서로 간섭하지 않고 같은 칸을 지날 수 있다
 * (PUZ_01 §5). 그런데 칸은 `fill` 색 하나만 가지므로, 두 색이 겹친 칸은 나중에 칠한
 * 색 하나만 보였다 - "빨강과 파랑이 같이 지나는데 파랑만 보인다". 그래서 겹친 칸은
 * 색의 개수만큼 칸을 나눠 **모든 색을 한 번에** 보여 준다.
 *
 * 띠의 방향은 광선이 나란히 달리는 것처럼 보이게 고른다. 가로로 지나는 광선은 위아래로
 * 쌓인 가로 띠가, 세로로 지나는 광선은 좌우로 늘어선 세로 띠가 나란한 광선으로 읽힌다.
 */
export enum EBoardStripeOrientation {
	/** 가로 띠 - 위에서 아래로 쌓인다. 첫 색이 맨 위다 */
	HORIZONTAL = 'HORIZONTAL',
	/** 세로 띠 - 왼쪽에서 오른쪽으로 늘어선다. 첫 색이 맨 왼쪽이다 */
	VERTICAL = 'VERTICAL',
}

/**
 * 표현 계층이 미리 만들어 두는 띠의 개수 상한.
 *
 * Custom UI 트리는 한 번만 만들어지므로 띠 노드도 칸마다 이만큼 고정으로 둔다.
 * 레이저의 색이 3종(RED/GREEN/BLUE)이라 3이면 전부 겹쳐도 담긴다. 이보다 긴
 * 목록은 앞의 세 색만 그려진다.
 */
export const PUZZLE_BOARD_MAX_STRIPES = 3;

/** 띠 없음 - 칸을 `fill` 하나로 칠한다. 공유 상수라 **절대 변경하지 않는다** */
export const NO_STRIPES: readonly PuzzleBoardColor[] = [];

export function isSameColorList(left: readonly PuzzleBoardColor[], right: readonly PuzzleBoardColor[]): boolean {
	if (left.length !== right.length) {
		return false;
	}
	for (let index = 0; index < left.length; index++) {
		if (isSameColor(left[index], right[index]) === false) {
			return false;
		}
	}
	return true;
...
export type PuzzleBoardCellView = {
	...
	/**
	 * 칸을 같은 비율로 나눠 칠할 색 목록. 비어 있으면 `fill` 하나로 칠한다.
	 *
	 * 비어 있지 않으면 표현 계층은 색의 개수만큼 칸을 나눠 **그림과 `fill` 위에** 띠를
	 * 덮는다 - 겹친 광선처럼 "여기를 여러 색이 같이 지난다" 를 보이기 위한 값이라,
	 * 띠가 있는 칸에서는 그림이 가려진다. 순서대로 위(또는 왼쪽)부터 놓인다.
	 * `PUZZLE_BOARD_MAX_STRIPES` 를 넘는 색은 그려지지 않는다.
	 */
	stripes: readonly PuzzleBoardColor[],
	/** 띠를 놓는 방향. `stripes` 가 비어 있으면 의미가 없다 */
...
export function createCellView(): PuzzleBoardCellView {
	return {
		...
		stripes: NO_STRIPES,
		stripeOrientation: EBoardStripeOrientation.HORIZONTAL,
	};
}
...
export function applyCellPatch(cell: PuzzleBoardCellView, patch: PuzzleBoardCellPatch): PuzzleBoardCellView | undefined {
	...
	if ((patch.isVisible === undefined || patch.isVisible === cell.isVisible)
		...
		&& (patch.stripes === undefined || isSameColorList(patch.stripes, cell.stripes))
		&& (patch.stripeOrientation === undefined || patch.stripeOrientation === cell.stripeOrientation)) {
		return undefined;
	}
	return {
		...
		stripes: patch.stripes ?? cell.stripes,
		stripeOrientation: patch.stripeOrientation ?? cell.stripeOrientation,
	};
}
