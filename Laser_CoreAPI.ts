...
import { summarizeBeamCells } from 'Laser_BeamTracer';
...
export class LaserCoreAPI extends Component<typeof LaserCoreAPI> {
	...
	private constructSystems(): void {
		...
		console.log(`[LaserCoreAPI] Drag stream created (move driving ${this.props.continuousDrag ? 'on' : 'off'}). `
	...
	private onDragBegin(cell: number): void {
		...
		// 잡는 순간 바뀌는 것은 크리스탈이 있던 그 칸(떠오르며 GRABBED 테두리)뿐이다.
		// 예전에는 바탕 49칸 + 광선 + 기믹 + 트레이를 전부 다시 칠했다 - 첫 피드백이
		// 가장 무거운 리페인트 뒤에 나왔던 셈이다. 전환 더티 필터(이전 없음 -> 지금 칸)와 같다.
		this.applyDragTransitionVisuals(undefined, undefined);
	}
	...
	private onInventoryPick(slot: number): void {
		...
		this._dragStream?.notifyDragBegan();
		// 트레이에서 집는 순간 바뀌는 것은 **그 슬롯 하나(실루엣)** 뿐이다 - 크리스탈은 아직 판
		// 위에 오지 않았고 광선·기믹도 그대로다. 그런데 여기만 `applyBoardVisuals()` 로 남아 있어,
		// 손가락이 닿는 바로 그 순간에 바탕 49칸 + 광선 + 기믹까지 훑는 **가장 무거운 리페인트**가
		// 돌았다. 판에서 집는 쪽(`onDragBegin`)은 이미 전환 더티 필터로 바꿨으므로 같이 맞춘다.
		this._presenter.runBatch(() => this.applyInventoryVisuals());
	}
	...
	private onDragEnd(cell: number): void {
		// 집은 것이 없으면 마감할 것도 없다 - **먼저 도착한 릴리즈가 이긴다**.
		//
	...
	private applyBoardVisuals(trace?: LaserTraceResult): void {
		...
		// 리페인트 배치 커밋 - 덧칠의 중간 결과가 패널로 새지 않게 한다.
		// 바탕 리셋 → 층 겹치기 순서는 앞 프레임의 광선·실루엣을 지우는 데 필요하므로
		// 그대로 두고, 프레젠터가 패스 끝에 "실제로 달라진 칸" 만 한 번 알린다
		// (`PuzzleBoardPresenter.runBatch` 주석 · 가이드 분석 §6-1).
		this._presenter.runBatch(() => {
	 ...
	 *
	 * 세그먼트가 아니라 **칸 단위**로 돈다 (`summarizeBeamCells`). 색이 다른 광선은 층이
	 * 달라 같은 칸을 같이 지나는데(§5), 세그먼트를 그대로 돌면 나중에 칠한 색이 앞의 색을
	 * 덮어 한 색만 보였다. 지금은 한 칸을 지나는 색이 둘 이상이면 색의 개수만큼 칸을
	 * 같은 비율의 띠로 나눠 **모든 색을 함께** 보인다.
	 */
	private applyBeamVisuals(trace: LaserTraceResult): void {
		for (const cell of summarizeBeamCells(trace.segments)) {
			// 더티 필터가 걸려 있으면 그 칸만 기록한다 - 패치 객체를 만들기 전에 거른다 (§4.4)
			if (this._repaintFilter !== undefined
				&& this._repaintFilter.has(cell.row * BOARD_COL_COUNT + cell.col) === false) {
				continue;
			}
			const tints = cell.colors.map((color) => toneColor(getLaserColor(color), BEAM_TONE_SCALE));
			// 판 그림이 깔려 있으면 색만 바꿔서는 광선이 그림에 가려 보이지 않는다.
			// 그림째 광선 색으로 물들여야 지나간 자리가 드러난다.
			const tint = tints[0];
			if (tints.length === 1) {
				this.setCell(cell.row, cell.col, { fill: tint, tint: tint, label: '', isHighlighted: false });
				continue;
			}
			// 겹친 칸 - 띠 방향은 광선이 나란히 달리는 것처럼 보이게 진행 방향을 따른다
			this.setCell(cell.row, cell.col, {
				...
				isHighlighted: false,
				stripes: tints,
				stripeOrientation: getBeamStripeOrientation(cell.directions),
			});
		}
	}
	...
	private setCell(row: number, col: number, patch: {
		...
		/** 여러 색이 같이 지나는 광선 칸만 준다. 생략하면 띠를 지운다 (무늬와 같은 규칙) */
		stripes?: readonly PuzzleBoardColor[],
		stripeOrientation?: EBoardStripeOrientation,
	}): void {
		...
		this._presenter.setCell(row * BOARD_COL_COUNT + col, {
			...
			// 띠도 같은 규칙이다. 광선이 지나는 수신체·크리스탈 칸은 광선 다음에 덧칠되므로,
			// 여기서 지우지 않으면 겹친 광선의 띠가 오브젝트를 덮은 채 남는다
			stripes: patch.stripes ?? NO_STRIPES,
			stripeOrientation: patch.stripeOrientation ?? EBoardStripeOrientation.HORIZONTAL,
		});
	}
 ...
 * 겹친 광선 칸의 띠 방향.
 *
 * 광선이 전부 세로(UP/DOWN)로 지나면 세로 띠를 좌우로 늘어놓아 나란한 광선처럼 보이게
 * 하고, 가로·대각선·혼합이면 가로 띠를 위아래로 쌓는다.
 */
function getBeamStripeOrientation(directions: readonly ELaserDirection[]): EBoardStripeOrientation {
	const isVerticalOnly = directions.length > 0 && directions.every(
		(direction) => direction === ELaserDirection.UP || direction === ELaserDirection.DOWN);
	return isVerticalOnly ? EBoardStripeOrientation.VERTICAL : EBoardStripeOrientation.HORIZONTAL;
}
