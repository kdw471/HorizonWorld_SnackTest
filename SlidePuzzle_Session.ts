...
export class SlidePuzzleSession {
	...
	private beginQuest(quest: SlideMainTableEntry): boolean {
		this._quest = quest;
		this._roundIndex = 0;
	...
	private resolveLevelField<T>(fields: T[]): T | undefined {
		...
		if (field === undefined) {
			console.warn(`[SlidePuzzleSession] Level field ordinal ${ordinal} is out of range (${fields.length} fields). Falling back to random selection.`);
		}
		return field;
	}
	...
	private canResetRound(): boolean {
		return this._state === ESlidePuzzleState.ROUND_INTRO || this._state === ESlidePuzzleState.PLAYER_INPUT;
	}
	 ...
	 */
	private openLevel(level: SlidePuzzleLevel): void {
		this._level = level;
